...
def test_build_tavily_search_tool_raises_on_empty_domains(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """build_tavily_search_tool should raise ValueError for empty domain list."""
    monkeypatch.setenv("TAVILY_API_KEY", "test-key-dummy")

    with pytest.raises(ValueError, match="include_domains must not be empty"):
        tavily_module.build_tavily_search_tool(include_domains=[])
